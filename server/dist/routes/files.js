...
filesRouter.post('/', upload.single('file'), async (req, res) => {
    try {
        ...
        const uploaded = await driveService.uploadFile(req.file.originalname, req.file.mimetype, req.file.buffer);
        // Create metadata
 ...
 */
filesRouter.get('/:id/url', async (req, res) => {
    try {
        if (!req.accessToken) {
            return res.status(401).json({ error: 'Not authenticated' });
        }
 ...
 * Rename a file
 */
filesRouter.put('/:id', async (req, res) => {
    try {
        if (!req.accessToken) {
            return res.status(401).json({ error: 'Not authenticated' });
        }
